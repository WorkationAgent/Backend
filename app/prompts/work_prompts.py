...
- must_have에 헬스장·마트·병원 등 생활 인프라가 포함되어 있어도 무시하라 — 검색 키워드는 **작업 공간 장소 유형만** 생성한다.
...
[평가 범위]
여기서 평가하는 must_have 조건은 **작업 환경과 직접 관련된 것만**이다.
Wi-Fi, 콘센트, 조용한 환경, 작업 공간 유형 등이 해당한다.
헬스장, 마트, 교통, 숙소 시설 등 생활 인프라·숙소 조건은 평가하지 않는다 — 이것들은 다른 에이전트가 처리한다.

[평가 흐름]
1. 작업 공간이 0개이면: status=FAIL, 점수 항목 모두 0 (장소 자체가 없음)
2. 작업 환경 관련 must_have 조건을 확인하라 (생활 인프라 조건은 무시).
   - 하나라도 충족하지 못하면: status=FAIL, 점수는 실제 작업공간 품질 기반으로 계산
   - 완전 충족은 아니지만 대안 공간이 존재하면: status=CONDITIONAL_PASS
3. 작업 환경 관련 모든 필수 조건이 충족되면: status=PASS, 아래 기준으로 항목별 점수를 계산하라.
