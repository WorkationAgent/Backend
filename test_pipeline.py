...
async def test_planner_interpret() -> None:
    ...
    await _run_interpret(
        "한 달 정도 바다 근처에서 쉬고 싶어요. "
        "일은 안 할 거고 그냥 힐링하면서 맛집도 다니고 싶어요. "
        "연인이랑 같이 가는데 조용하고 감성적인 독채 숙소면 좋겠어요. "
        "예산은 좀 써도 괜찮아요."
    )
...
async def test_full_pipeline(raw_text: str) -> None:
    ...
    def _by_id(evals):
        ...
        if isinstance(evals, dict):
            evals = evals.get("living_evaluations") or evals.get("work_evaluations") or []
        return {str(e.accommodation_id): e for e in (evals or [])}

    living_map = _by_id(living_result.get("living_evaluations", []) if isinstance(living_result, dict) else [])
    work_map   = _by_id(work_result.get("work_evaluations", []) if isinstance(work_result, dict) else [])
    local_map  = {str(e.accommodation_id): e for e in (local_result if isinstance(local_result, list) else [])}
    ...
    for acc in raw_accommodations:
        ...
        print(f"\n{'─'*50}")
        print(f"  숙소: {acc['name']}")
        print(f"  주소: {acc.get('address', '')}")
        print(f"  Stay 점수: {acc['total_score']} — {acc.get('brief_reason', '')[:50]}")

        lv = living_map.get(acc_id)
        if lv:
            print(f"  Living 점수: {lv.score} (confidence {lv.confidence})")
            print(f"    → {lv.summary[:70] if lv.summary else ''}...")
            details = lv.details or {}
            for cat in ("transport", "grocery", "medical", "services"):
                d = details.get(cat, {})
                if d.get("found"):
                    print(f"    [{cat}] {d.get('nearest_minutes')}분 이내 ({d.get('zone_km')}km)")

        wk = work_map.get(acc_id)
        if wk:
            print(f"  Work 점수: {wk.score} (confidence {wk.confidence})")
            print(f"    → {wk.summary[:70] if wk.summary else ''}...")

        lc = local_map.get(acc_id)
        if lc:
            print(f"  Local 점수: {lc.score} (confidence {lc.confidence})")
            print(f"    → {lc.summary[:70] if lc.summary else ''}...")
            spots = lc.details.tourism_spots[:3] if lc.details.tourism_spots else []
            for spot in spots:
                print(f"    📍 {spot.name} ({int(spot.dist_m)}m)")


async def main() -> None:
    # 전체 파이프라인 테스트 (줄글 → 최종 추천)
